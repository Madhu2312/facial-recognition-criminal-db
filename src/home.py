...
pages = []
for i in range(4):
    pages.append(tk.Frame(root, bg="green"))
...
def basicPageSetup(pageNo):
    ...
    back_button = tk.Button(pages[pageNo], image=back_img, bg="green", bd=0, highlightthickness=0,
           activebackground="green", command=goBack)
    back_button.image = back_img
    back_button.place(x=10, y=10)

    heading = tk.Label(pages[pageNo], fg="white", bg="green", font="Arial 20 bold", pady=10)
    heading.pack()

    content = tk.Frame(pages[pageNo], bg="green", pady=20)
    content.pack(expand="true", fill="both")

    left_frame = tk.Frame(content, bg="green")
    left_frame.grid(row=0, column=0, sticky="nsew")

    right_frame = tk.LabelFrame(content, text="Detected Criminals", bg="green", font="Arial 16 bold", bd=4,
                             foreground="#2ea3ef", labelanchor="n")
    right_frame.grid(row=0, column=1, sticky="nsew", padx=14, pady=14)
...
def showImage(frame, img_size):
    ...
    if (img_label == None):
        img_label = tk.Label(left_frame, image=img, bg="green")
        img_label.image = img
...
def selectMultiImage(opt_menu, menu_var):
    ...
    if(len(path_list) < 5):
        ...
        slide_control_panel = tk.Frame(left_frame, bg="green", pady=20)
        slide_control_panel.pack()

        back_img = tk.PhotoImage(file="previous.png")
        next_img = tk.PhotoImage(file="next.png")

        prev_slide = tk.Button(slide_control_panel, image=back_img, bg="green", bd=0, highlightthickness=0,
                            activebackground="green", command=lambda : getNewSlide("prev"))
        ...
        next_slide = tk.Button(slide_control_panel, image=next_img, bg="green", bd=0, highlightthickness=0,
                            activebackground="green", command=lambda : getNewSlide("next"))
...
def getPage1():
    ...
    btn_grid = tk.Frame(left_frame, bg="green")
    btn_grid.pack()
    ...
    canvas = tk.Canvas(right_frame, bg="green", highlightthickness=0)
    canvas.pack(side="left", fill="both", expand="true", padx=30)
    scrollbar = tk.Scrollbar(right_frame, command=canvas.yview, width=20, troughcolor="green", bd=0,
                          activebackground="#00bcd4", bg="#2196f3", relief="raised")
    scrollbar.pack(side="left", fill="y")

    scroll_frame = tk.Frame(canvas, bg="green", pady=20)
    ...
    tk.Label(scroll_frame, text="* Required Fields", bg="green", fg="yellow", font="Arial 13 bold").pack()
    # Adding Input Fields
    ...
    for i, field in enumerate(input_fields):
        row = tk.Frame(scroll_frame, bg="green")
        row.pack(side="top", fill="x", pady=15)

        label = tk.Text(row, width=20, height=1, bg="green", fg="#ffffff", font="Arial 13", highlightthickness=0, bd=0)
...
def showCriminalProfile(name):
    top = tk.Toplevel(bg="green")
    top.title("Criminal Profile")
    top.geometry("1500x900+%d+%d"%(root.winfo_x()+10, root.winfo_y()+10))

    tk.Label(top, text="Criminal Profile", fg="white", bg="green", font="Arial 20 bold", pady=10).pack()

    content = tk.Frame(top, bg="green", pady=20)
    content.pack(expand="true", fill="both")
    ...
    img = Image.fromarray(img)
    img = ImageTk.PhotoImage(img)
    img_label = tk.Label(content, image=img, bg="green")
    img_label.image = img
    img_label.grid(row=0, column=0)

    info_frame = tk.Frame(content, bg="green")
    info_frame.grid(row=0, column=1, sticky='w')

    for i, item in enumerate(crim_data.items()):
        tk.Label(info_frame, text=item[0], pady=15, fg="yellow", font="Arial 15 bold", bg="green").grid(row=i, column=0, sticky='w')
        tk.Label(info_frame, text=":", fg="yellow", padx=50, font="Arial 15 bold", bg="green").grid(row=i, column=1)
        val = "---" if (item[1]=="") else item[1]
        tk.Label(info_frame, text=val.capitalize(), fg="white", font="Arial 15", bg="green").grid(row=i, column=2, sticky='w')
...
######################################## Home Page ####################################
tk.Label(pages[0], text="CRIMINAL FACIAL DETECTION AND OCCURENCE PREDICITION ", fg="white", bg="green",
      font="Arial 20 bold", pady=30).pack()

logo = tk.PhotoImage(file = "logo.png")
tk.Label(pages[0], image=logo, bg="green").pack()

btn_frame = tk.Frame(pages[0], bg="green", pady=30)
btn_frame.pack()
...
for btn in btn_frame.winfo_children():
    btn.configure(font="Arial 20", width=15, bg="#2196f3", fg="white",
        pady=13, bd=0, highlightthickness=0, activebackground="#091428", activeforeground="white")
    btn.pack(pady=14)
